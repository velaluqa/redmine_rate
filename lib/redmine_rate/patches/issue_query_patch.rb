  ...
  module Patches
    module IssueQueryPatch
      def self.included(base)
        ...
        base.class_eval do
          alias_method_chain :available_columns, :cost
        end
      end
      ...
      module InstanceMethods
        def available_columns_with_cost
          return @available_columns if @available_columns
          available_columns_without_cost
        ...
        def total_for_cost_with_currency(scope)
          total = if group_by_column.try(:name) == :project
                    ...
                    scope.joins("JOIN (SELECT issue_id, cost FROM #{TimeEntry.table_name}) AS joined_time_entries ON joined_time_entries.issue_id = #{Issue.table_name}.id").
